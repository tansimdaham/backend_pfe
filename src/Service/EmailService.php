...
L\'équipe PharmaLearn';
     ...
     *
     * @param Utilisateur $user L'utilisateur qui a demandé la réinitialisation
     ...
     */
    public function sendPasswordResetEmail(Utilisateur $user, string $resetToken): bool
    {
        try {
            // Logs détaillés pour le suivi de l'envoi d'email
            $logPrefix = '[EMAIL-SERVICE] [PASSWORD-RESET]';
            error_log($logPrefix . ' Début de l\'envoi de l\'email de réinitialisation à ' . $user->getEmail());
            error_log($logPrefix . ' Préparation de l\'email de réinitialisation pour ' . $user->getEmail());

            // URL pour réinitialiser le mot de passe
            $resetUrl = 'http://localhost:5174/reset-password/' . $resetToken;

            // Version texte pour les clients qui ne supportent pas le HTML
            $textContent = 'Bonjour ' . $user->getName() . ',

Vous avez demandé la réinitialisation de votre mot de passe sur PharmaLearn.
...
L\'équipe PharmaLearn';
            ...
            <body>
                <div class="email-container">
                    ...
                    <div class="email-body">
                        ...
                        <div class="message">
                            <p>Vous avez demandé la réinitialisation de votre mot de passe sur PharmaLearn.</p>
                            <p>Cliquez sur le bouton ci-dessous pour créer un nouveau mot de passe. Ce lien est valable pendant <strong>1 heure</strong>.</p>
                        </div>
                        ...
                        <div class="info-box">
                            ...
                            <p>Si vous n\'avez pas demandé cette réinitialisation, vous pouvez ignorer cet email. Votre compte reste sécurisé.</p>
                        </div>
            ...
            $email = (new Email())
                ->from(new Address($this->senderEmail, 'PharmaLearn'))
                ->to($user->getEmail())
                ->subject('Réinitialisation de votre mot de passe PharmaLearn')
                ->text($textContent)
                ->html($htmlContent);
            ...
            error_log($logPrefix . ' ✅ Email de réinitialisation envoyé avec succès à ' . $user->getEmail());

            return true;
        } catch (\Exception $e) {
            $errorMsg = $logPrefix . ' ERREUR lors de l\'envoi de l\'email de réinitialisation: ' . $e->getMessage();
            error_log($errorMsg);
            error_log($logPrefix . ' Trace: ' . $e->getTraceAsString());
            error_log($logPrefix . ' ❌ ÉCHEC de l\'envoi de l\'email de réinitialisation à ' . $user->getEmail());
            error_log($logPrefix . ' ❌ Erreur: ' . $e->getMessage());
